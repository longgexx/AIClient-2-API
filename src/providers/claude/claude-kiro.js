...
import {
    getCacheEstimatorForAccount,
    clearCacheEstimatorsForAccount,
    clearCacheEstimatorsForSession
} from './cache-estimator.js';

// 重新导出缓存推测相关函数，保持向后兼容
export { clearCacheEstimatorsForAccount, clearCacheEstimatorsForSession };
...
// ============ 性能优化：预编译正则表达式 ============
// 用于 parseAwsEventStreamBuffer - 单次扫描匹配所有 JSON 起始模式
const AWS_JSON_START_PATTERN = /\{"(?:content|name|followupPrompt|input|stop|contextUsagePercentage)":/g;

// 用于 parseEventStreamChunk - 预编译 SSE 事件正则
const SSE_EVENT_REGEX = /:message-typeevent(\{[^]*?(?=:event-type|$))/g;
const LEGACY_EVENT_REGEX = /event(\{.*?(?=event\{|$))/gs;

// 用于工具调用模式匹配的缓存
const toolCallPatternCache = new Map();
function getToolCallRemovalPattern(funcName) {
    if (!toolCallPatternCache.has(funcName)) {
        const escaped = funcName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        // 使用更简单的非贪婪匹配，避免指数级回溯
        toolCallPatternCache.set(funcName,
            new RegExp(`\\[Called\\s+${escaped}\\s+with\\s+args:\\s*\\{[^\\]]*\\}\\]`, 'g')
        );
    }
    return toolCallPatternCache.get(funcName);
}
...
async initializeAuth(forceRefresh = false) {
    ...
    parseEventStreamChunk(rawData) {
        ...
        // 性能优化：使用预编译的正则表达式（在模块顶部定义）
        // 重置 lastIndex 以确保从头开始匹配
        SSE_EVENT_REGEX.lastIndex = 0;
        LEGACY_EVENT_REGEX.lastIndex = 0;

        // 首先尝试使用 SSE 格式解析
        let matches = [...rawStr.matchAll(SSE_EVENT_REGEX)];

        // 如果 SSE 格式没有匹配到，回退到旧的格式
        if (matches.length === 0) {
            matches = [...rawStr.matchAll(LEGACY_EVENT_REGEX)];
        }
        ...
        if (bracketToolCalls) {
            ...
            // 性能优化：使用缓存的正则表达式移除工具调用文本
            for (const tc of bracketToolCalls) {
                const pattern = getToolCallRemovalPattern(tc.function.name);
     ...
     *
     * 性能优化：使用预编译正则表达式单次扫描，替代原来的 6 次 indexOf 搜索
     */
    parseAwsEventStreamBuffer(buffer) {
        const events = [];
        let lastProcessedEnd = 0;

        // 重置正则的 lastIndex（因为是全局正则）
        AWS_JSON_START_PATTERN.lastIndex = 0;

        let match;
        while ((match = AWS_JSON_START_PATTERN.exec(buffer)) !== null) {
            const jsonStart = match.index;

            // 使用括号计数法找到 JSON 结束位置
            const jsonEnd = this._findJsonEndOptimized(buffer, jsonStart);

            if (jsonEnd < 0) {
                // 不完整的 JSON，保留从 jsonStart 开始的部分等待更多数据
            ...
            // 更新正则的搜索位置，避免重复匹配
            AWS_JSON_START_PATTERN.lastIndex = lastProcessedEnd;
        }

        // 返回未处理的剩余部分
        const remaining = lastProcessedEnd > 0 ? buffer.substring(lastProcessedEnd) : buffer;
        // 如果没有找到任何 JSON，检查是否有潜在的不完整 JSON 开头
        if (events.length === 0 && remaining.length > 0) {
            // 查找最后一个可能的 JSON 开头位置
            const lastBrace = remaining.lastIndexOf('{"');
            if (lastBrace >= 0) {
                return { events, remaining: remaining.substring(lastBrace) };
            }
        }

        return { events, remaining: events.length > 0 ? remaining : '' };
    }

    /**
     * 优化的 JSON 结束位置查找
     * 使用括号计数法，正确处理嵌套和字符串内的括号
     */
    _findJsonEndOptimized(buffer, jsonStart) {
        let braceCount = 0;
        let inString = false;
        let escapeNext = false;
    ...
    async * generateContentStream(model, requestBody) {
        ...
        try {
            ...
            // 性能优化：使用索引追踪缓冲区位置，减少字符串分配
            let bufferOffset = 0;
            const COMPACT_THRESHOLD = 8192; // 8KB 时压缩缓冲区
            ...
            for await (const event of this.streamApiReal('', finalModel, requestBody)) {
                if (event.type === 'contextUsage' && event.contextUsagePercentage) {
                    ...
                    const events = [];
                    // 性能优化：使用有效长度而非每次 slice
                    const effectiveLength = streamState.buffer.length - bufferOffset;

                    while (effectiveLength > 0) {
                        const effectiveBuffer = bufferOffset > 0
                            ? streamState.buffer.substring(bufferOffset)
                            : streamState.buffer;

                        if (!streamState.inThinking && !streamState.thinkingExtracted) {
                            const startPos = findRealTag(effectiveBuffer, KIRO_THINKING.START_TAG);
                            if (startPos !== -1) {
                                const before = effectiveBuffer.substring(0, startPos);
                                if (before) events.push(...createTextDeltaEvents(before));

                                bufferOffset += startPos + KIRO_THINKING.START_TAG.length;
                                streamState.inThinking = true;
                                continue;
                            }

                            const safeLen = Math.max(0, effectiveBuffer.length - KIRO_THINKING.START_TAG.length);
                            if (safeLen > 0) {
                                const safeText = effectiveBuffer.substring(0, safeLen);
                                if (safeText) events.push(...createTextDeltaEvents(safeText));
                                bufferOffset += safeLen;
                            }
                            break;
                        }

                        if (streamState.inThinking) {
                            const endPos = findRealTag(effectiveBuffer, KIRO_THINKING.END_TAG);
                            if (endPos !== -1) {
                                const thinkingPart = effectiveBuffer.substring(0, endPos);
                                if (thinkingPart) events.push(...createThinkingDeltaEvents(thinkingPart));

                                bufferOffset += endPos + KIRO_THINKING.END_TAG.length;
                                streamState.inThinking = false;
                                ...
                                // 检查并跳过换行符
                                const remaining = streamState.buffer.substring(bufferOffset);
                            ...
                            const safeLen = Math.max(0, effectiveBuffer.length - KIRO_THINKING.END_TAG.length);
                            if (safeLen > 0) {
                                const safeThinking = effectiveBuffer.substring(0, safeLen);
                        ...
                        if (streamState.thinkingExtracted) {
                            const rest = effectiveBuffer;
                            bufferOffset = streamState.buffer.length; // 标记全部已处理
                    ...
                    if (bufferOffset > COMPACT_THRESHOLD) {
                        streamState.buffer = streamState.buffer.substring(bufferOffset);
                        bufferOffset = 0;
                    }
     ...
     *
     * 性能优化：批量收集文本后一次性计数，减少 tokenizer 调用次数
     */
    /**
    ...
    estimateInputTokens(requestBody) {
        // 性能优化：收集所有文本片段，最后一次性计数
        const textParts = [];
        let fixedTokens = 0; // 固定 token 数（如图片）

        // Collect system prompt text
        if (requestBody.system) {
            textParts.push(this.getContentText(requestBody.system));
            // Collect cache_control in system if present
            if (Array.isArray(requestBody.system)) {
                for (const part of requestBody.system) {
                    if (part.cache_control) {
                        textParts.push(JSON.stringify(part.cache_control));
                    }
                }
            }
        }
        ...
        if (requestBody.thinking?.type === 'enabled') {
            ...
            textParts.push(`<thinking_mode>enabled</thinking_mode><max_thinking_length>${budget}</max_thinking_length>`);
        }

        // Collect all messages text
                    ...
                    if (Array.isArray(message.content)) {
                        for (const part of message.content) {
                            ...
                            if (part.type === 'text' && part.text) {
                                ...
                                // Collect tool_use_id
                                if (part.tool_use_id) {
                                    textParts.push(part.tool_use_id);
                                }
                                textParts.push(this.getContentText(part.content));
                            } else if (part.type === 'tool_use') {
                                // Collect tool name and id
                                if (part.name) {
                                    textParts.push(part.name);
                                }
                                if (part.id) {
                                    textParts.push(part.id);
                                }
                                if (part.input) {
                                ...
                                // 文档根据 base64 数据估算
                                if (part.source?.data) {
                                    const estimatedChars = part.source.data.length * 0.75;
                                    fixedTokens += Math.ceil(estimatedChars / 4);
                                }
                            }
                        }
                    } else {
                        textParts.push(this.getContentText(message));
                    }
                }
            }
        }

        // Collect tools definitions if present
        if (requestBody.tools && Array.isArray(requestBody.tools)) {
            textParts.push(JSON.stringify(requestBody.tools));
        }

        // 一次性计数所有文本（用换行符连接，对 token 计数影响很小）
        const combinedText = textParts.filter(t => t).join('\n');
        return this.countTextTokens(combinedText) + fixedTokens;
    }
